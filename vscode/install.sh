# after vscode is installed with `code` command working...

# run these to install necessary packages
code --install-extension apollographql.vscode-apollo
code --install-extension dsznajder.es7-react-js-snippets
code --install-extension justusadam.language-haskell
code --install-extension kumar-harsh.graphql-for-vscode
code --install-extension ms-azuretools.vscode-docker
code --install-extension ms-python.python
code --install-extension naumovs.theme-oceanicnext

# find `settings.json` here:
cp -rp ./settings.json /Users/$USER/Library/ApplicationSupport/Code/User/settings.json

# find `keybindings.json` here:
# cp -rp ./keybindings.json /Users/$USER/Library/ApplicationSupport/Code/User/keybindings.json